...
import { X, Camera, Loader2, AlertCircle, QrCode } from "lucide-react";
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      if (isBarcodeDetectorSupported()) {
        // Use BarcodeDetector API (Chrome Android, Edge)
        ...
        // Fallback: Canvas + jsQR-like pixel decode not available
        // We keep the live camera preview — user sees we tried
        setError("Scanner automatique non disponible sur cet appareil. Utilisez la saisie manuelle.");
        stopCamera();
        setScanning(false);
      }
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 shrink-0">
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="font-bold text-sm uppercase tracking-wider">Scanner QR Code</span>
        </div>
        <button
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        {error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center bg-black">
            ...
            <button
              onClick={() => { stopCamera(); onClose(); }}
              className="mt-2 px-6 py-2.5 rounded-xl bg-primary text-black font-bold text-sm"
            >
              Fermer
            </button>
          </div>
      ...
      {scanning && !error && (
        <div className="px-4 py-4 text-center shrink-0">
          <p className="text-white/60 text-xs">Pointez la caméra vers le QR du joueur</p>
          <button
            onClick={() => { stopCamera(); onClose(); }}

...
import { LayoutDashboard, Ticket as TicketIcon, Users, Trophy, Coins, LogOut, UserCheck, Settings, QrCode } from "lucide-react";
      ...
      <aside className="w-64 border-r bg-sidebar text-sidebar-foreground flex flex-col shrink-0">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
                  isActive
        ...
        {user && (
          <div className="p-4 border-t border-sidebar-border">
            <div className="text-xs text-sidebar-foreground/70 font-medium mb-0.5">{user.username}</div>
            <div className="text-xs text-sidebar-foreground/40 mb-2">{user.email}</div>
            <button
              onClick={() => logout()}
